...
class DtrController extends Controller
{
    public function index() {
        ...
        $Months=array();
        $Monthnow = Carbon::createFromDate(now())->format('f');

        for ($m=1; $m<=12; $m++) {
            $Months[] = array($m,date('F', mktime(0,0,0,$m, 1, date('Y'))));
       }

       

        return view('user.daily-time-record.index',compact('Years','Months'));
    ...
    public function print(Request $request) {
        ...
        $this->authorize('userview', \App\Models\Dtr_History::class);
        ...
        $Employee = Employee::where('email','=',auth()->user()->email)->with('Office')->with('Section')->with('Unit')->get()->first(); 
        $Dtrs = Dtr_History::where('employeeid','=', $Employee->id)->whereBetween('date',[$request->fromdate, $request->todate])->with('Employee')->with('User')->orderby('time', 'asc')->get();
        ...
        while ($startDate->lte($endDate)){
          ...
          $Leaves = Leave::where('employeeid','=', $Employee->id)->with('Employee','Leave_Type')->get();
          {
         ...
         $TravelOrders = TravelOrder::where('employeeid','=', $Employee->id)->with('Employee')->get();
          {
          ...
          $times = Dtr_History::where('employeeid','=', $Employee->id)->where('date',$startDate)->with('Employee')->with('User')->orderby('time', 'asc')->get();
...
//
        ...
        return view('user.daily-time-record.dtrtable',compact('Employee','Dtrs','all_dates','all_dtr','lates','uts','FinalLeaves','FinalTravelOrders','FinalDateRage','FinalEvent'));
        }
     
        public function store(Request $request) {

          $this->authorize('create', \App\Models\DtrRequest::class);
            ...
            {
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'TIME IN - MORNING')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              } 
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'MORNING')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              }
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'EVENT - MORNING')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              }
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'EVENT - WHOLE DAY')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              }
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'WHOLE DAY')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              }
            }
            ...
            {
              $check = dtr_history::where('employeeid','=',$Employee->id)->where('date','=', $request->date)->where('schedule','=', 'TIME IN - AFTERNOON')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              } 
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'AFTERNOON')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              }
              $check = dtr_history::where('employeeid','=',$Employee->id)->where('date','=', $request->date)->where('schedule','=', 'EVENT - AFTERNOON')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              }
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'EVENT - WHOLE DAY')->get()->first();
            ...
            {
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'TIME OUT - MORNING')->get()->first();
              if ($check) {
                return back()->with('DTRError4','Error')->withInput();
              } 
              $check = dtr_history::where('employeeid','=', $Employee->id)->where('date','=', $request->date)->where('schedule','=', 'MORNING')->get()->first();
            ...
            if ($check) {
              return redirect()->route('my-daily-time-record.index')->with('DTRError1', 'Duplicate DTR Entry!')->withInput();
            }        
                     
      
                      DtrRequest::create($formfields) ; 
                      return back()->with('message', 'DTR Requested Succesfully!')->withInput();
                    }      
   public function request() {
       
    $this->authorize('create', \App\Models\DtrRequest::class);
        
    return view('user.daily-time-record.myrequest');
